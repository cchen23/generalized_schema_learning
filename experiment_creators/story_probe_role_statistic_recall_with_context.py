...
def write_examples(fillers_by_role_dict,
                   ...
                   context_a_wordlist_index,
                   context_b_wordlist_index,
                   save_path,
                   ambiguous=None):
    X = np.empty((0, story_frame_matrix.shape[1] + 3, story_frame_matrix.shape[2]))
    y = np.empty((0, 1))
    noise_matrix = np.reshape(np.array([noise_wordlist_index]), (1, 1, 1))

    if ambiguous == 'all':
        for context_wordlist_index in [context_a_wordlist_index, context_b_wordlist_index]:
            context_matrix = np.reshape(np.array([context_wordlist_index]), (1, 1, 1))
            for role in roles:
                story = np.copy(story_frame_matrix)
                story[0, :] = noise_wordlist_index
                story = np.concatenate((context_matrix, story, noise_matrix, np.reshape(question_wordlist_indices[role], (1, 1, 1))), axis=1)
        ...
        for i in range(num_examples):
            if i < num_examples // 2:
                context = 'A'
                context_matrix = np.reshape(np.array([context_a_wordlist_index]), (1, 1, 1))
            else:
                context = 'B'
                context_matrix = np.reshape(np.array([context_b_wordlist_index]), (1, 1, 1))
            story = np.copy(story_frame_matrix)
            for role in roles:
                filler = np.random.choice(fillers_by_role_dict[context][role])
            ...
            story_with_noise = np.concatenate((context_matrix, story, noise_matrix), axis=1)
...
def generate_experiments(num_dims=50,
    ...
    wordlist += [NOISE_WORD, CONTEXT_A_WORD, CONTEXT_B_WORD]
    ...
    context_a_wordlist_index = wordlist.index(CONTEXT_A_WORD)
    context_b_wordlist_index = wordlist.index(CONTEXT_B_WORD)
    ...
    write_examples(fillers_by_role_dict=train_fillers_by_role_dict,
                   ...
                   context_a_wordlist_index=context_a_wordlist_index,
                   context_b_wordlist_index=context_b_wordlist_index,
    ...
    test_flipped_context_distribution_fillers_by_role_dict = {
            'A': {
                'emcee': add_05_fillers_indices_train,
                'friend': add_05_fillers_indices_train,
                'poet': subtract_05_fillers_indices_train,
    ...
    write_examples(fillers_by_role_dict=test_flipped_context_distribution_fillers_by_role_dict,
                   story_frame_matrix=story_frame_matrix,
                   num_examples=num_test_examples,
                   roles=ROLES,
                   role_story_indices=role_story_indices,
                   ...
                   context_a_wordlist_index=context_a_wordlist_index,
                   context_b_wordlist_index=context_b_wordlist_index,
                   save_path=os.path.join(save_dir, "test_flipped_context_distribution.p"))

    # Generate unseen flipped_context_distribution test set.
    unseen_flipped_context_distribution_fillers_by_role_dict = {
            'A': {
                'emcee': add_05_fillers_indices_test,
                'friend': add_05_fillers_indices_test,
                'poet': subtract_05_fillers_indices_test,
    ...
    write_examples(fillers_by_role_dict=unseen_flipped_context_distribution_fillers_by_role_dict,
                   story_frame_matrix=story_frame_matrix,
                   num_examples=num_test_examples,
                   roles=ROLES,
                   role_story_indices=role_story_indices,
                   ...
                   context_a_wordlist_index=context_a_wordlist_index,
                   context_b_wordlist_index=context_b_wordlist_index,
                   save_path=os.path.join(save_dir, "test_unseen_flipped_context_distribution.p"))

    # Generate unseen ood test set.
    unseen_no_addition_distribution_fillers_by_role_dict = {
            'A': {
    ...
    write_examples(fillers_by_role_dict=unseen_no_addition_distribution_fillers_by_role_dict,
                   story_frame_matrix=story_frame_matrix,
                   num_examples=num_test_examples,
                   roles=ROLES,
                   role_story_indices=role_story_indices,
