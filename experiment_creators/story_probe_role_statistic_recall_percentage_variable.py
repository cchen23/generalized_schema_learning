...
def generate_experiments(num_dims=50,
    ...
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
    STORY_FRAME = "begin story sit subject friend announce emcee perform poet consume dessert drink goodbye".split(" ")
    ...
    # Get wordlist.
    add_05_even_fillers_train = ['add_05_even_train' + str(filler_index) for filler_index in range(num_train_fillers_per_category)]
    add_05_even_fillers_test = ['add_05_even_test' + str(filler_index) for filler_index in range(num_test_fillers_per_category)]
    add_05_even_fillers = add_05_even_fillers_train + add_05_even_fillers_test
    
    add_05_odd_fillers_train = ['add_05_odd_train' + str(filler_index) for filler_index in range(num_train_fillers_per_category)]
    add_05_odd_fillers_test = ['add_05_odd_test' + str(filler_index) for filler_index in range(num_test_fillers_per_category)]
    add_05_odd_fillers = add_05_odd_fillers_train + add_05_odd_fillers_test
    no_addition_fillers = [str(filler_index) for filler_index in range(num_test_fillers_per_category)]
    fillers = add_05_even_fillers + add_05_odd_fillers + no_addition_fillers
    ...
    distribution_A_fillers_train = list(np.random.choice(add_05_even_fillers_train, size=num_train_fillers_per_category * percentage // 100, replace=False)) + list(np.random.choice(add_05_odd_fillers_train, size=num_train_fillers_per_category * (100 - percentage) // 100, replace=False))
    distribution_A_fillers_test = list(np.random.choice(add_05_even_fillers_test, size=num_test_fillers_per_category * percentage // 100, replace=False)) + list(np.random.choice(add_05_odd_fillers_test, size=num_test_fillers_per_category * (100 - percentage) // 100, replace=False))
    distribution_B_fillers_train = list(np.random.choice(add_05_odd_fillers_train, size=num_train_fillers_per_category * percentage // 100, replace=False)) + list(np.random.choice(add_05_even_fillers_train, size=num_train_fillers_per_category * (100 - percentage) // 100, replace=False))
    distribution_B_fillers_test = list(np.random.choice(add_05_odd_fillers_test, size=num_test_fillers_per_category * percentage // 100, replace=False)) + list(np.random.choice(add_05_even_fillers_test, size=num_test_fillers_per_category * (100 - percentage) // 100, replace=False))
    ...
    distribution_A_fillers_indices_train = [wordlist.index(filler) for filler in distribution_A_fillers_train]
    distribution_B_fillers_indices_train = [wordlist.index(filler) for filler in distribution_B_fillers_train]
    distribution_A_fillers_indices_test = [wordlist.index(filler) for filler in distribution_A_fillers_test]
    distribution_B_fillers_indices_test = [wordlist.index(filler) for filler in distribution_B_fillers_test]
    ...
    for i in range(len(wordlist)):
        ...
        if "add_05_even" in word:
            word_embedding['vector'] = create_word_vector("add05even", normalize_filler_distribution=normalize_filler_distribution)
        elif "add_05_odd" in word:
            word_embedding['vector'] = create_word_vector("add05odd", normalize_filler_distribution=normalize_filler_distribution)
        elif word == 'zzz':
