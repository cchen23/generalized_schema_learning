...
def train(num_training_iterations, report_interval):
  ...
  output_logits = run_model(dataset_tensors.observations, dataset.target_size) # logits. dataset_tensors.observations=self.X, dataset.target_size = self.y from gru_model
  ...
  train_loss = dataset.cost(output_logits, dataset_tensors.target,
                            dataset_tensors.mask) # loss

  # Set up optimizer with global norm clipping.
  trainable_variables = tf.trainable_variables()
  grads, _ = tf.clip_by_global_norm(
      tf.gradients(train_loss, trainable_variables), FLAGS.max_grad_norm) # grads, norm
  ...
  optimizer = tf.train.RMSPropOptimizer(
      FLAGS.learning_rate, epsilon=FLAGS.optimizer_epsilon) # optimizer
  train_step = optimizer.apply_gradients(
      zip(grads, trainable_variables), global_step=global_step) # update
