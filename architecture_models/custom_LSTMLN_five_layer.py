...
class lstmln_model(object):
    ...
    def __init__(self, FLAGS):
        ...
        self.lstm_4 = BasicLSTMCell(FLAGS.num_hidden_units)
        self.lstm_5 = BasicLSTMCell(FLAGS.num_hidden_units)
        ...
        with tf.variable_scope("lstmln_step") as scope:
            for t in range(0, FLAGS.input_dim):
                ...
                self.new_h, self.new_state, concat = self.lstm_3(self.X[:, t, :], self.new_state, scope="lstm4")
                self.new_h, self.new_state, concat = self.lstm_3(self.X[:, t, :], self.new_state, scope="lstm5")
                if t > 0:
