...
def write_run_training(experiment_name, filler_type, architecture_name, num_epochs, trial_num, regime=None):
    ...
    f.write("#SBATCH -J 'run_fastweights_runexperiments_%s_%s_%s_%dgpu'\n" % (experiment_filename, architecture_filename, fillertype_filename, trial_num))
    f.write("#SBATCH -o outputs/slurm-%%j-run_fastweights_runexperiments_%s_%s_%s_%dgpu.out\n" % (experiment_filename, architecture_filename, fillertype_filename, trial_num))
    ...
    f.write("python %srun_experiment.py --function=train --exp_name=%s --filler_type=%s --model_name=%s --num_epochs=%d --trial_num=%d " % (base_dir, experiment_name, filler_type, architecture_name, num_epochs, trial_num))
    if "curriculum" in experiment_name:
        f.write("--regime=%s " % regime)
    f.write("> outputs/$(date +\"%%Y%%m%%d%%H%%M\")_runexperiments_%s_%s_%s_%dgpu.txt\n" % (experiment_filename, filler_type, architecture_filename, trial_num))
    f.write("date\n")
...
def write_run_analysis(experiment_name, filler_type, test_filename, trial_num):
    ...
    f.write("#!/usr/bin/env bash\n")
    f.write("#SBATCH -J 'run_fastweights_runanalysis_%s_%s_%s_%dgpu'\n" % (experiment_filename, fillertype_filename, test_filename[:-2], trial_num))
    f.write("#SBATCH -o outputs/slurm-%%j-run_fastweights_runanalysis_%s_%s_%s_%dgpu.out\n" % (experiment_filename, fillertype_filename, test_filename[:-2], trial_num))
    f.write("#SBATCH -p all\n")
...
if __name__ == '__main__':
    experiment_name = "variablefiller_gensymbolicstates_100000_1_testunseen_QSubjectQFriend"
