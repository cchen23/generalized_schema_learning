...
def plot_data(data, epochs_dict, plot_index, experiment_title, experiment_title2, chance_rate, save=False, save_dir=None):
    ...
    if save:
        plt.savefig(os.path.join(save_dir, 'multitrial_barchart_%s%s_%s' % (experiment_title.replace("_","").replace(" ","").replace(",",""), experiment_title2.replace("_","").replace(" ","").replace(",",""), info_type.replace(" ",""))), bbox_inches="tight", dpi=900)
    # plt.show()
    plt.close()
...
def plot_data_double(data0, data1, label0, label1, epochs_dict, experiment_title, experiment_title2, chance_rate, save=False, save_dir=None, savename="both"):
    ...
    if save:
        plt.savefig(os.path.join(save_dir, 'multitrial_barchart_%s%s_%s' % (experiment_title.replace("_","").replace(" ","").replace(",",""), experiment_title2.replace("_","").replace(" ","").replace(",",""), savename)), bbox_extra_artists=(legend,), bbox_inches="tight", dpi=900)
    # plt.show()
    plt.close()

def get_color_alpha(query):
    role_colors = {"QDessert":"#1f77b4", "QDrink":"#ff7f0e", "QEmcee":"#2ca02c", "QFriend":"#d62728", "QPoet":"#9467bd", "QSubject":"#8c564b"}
    query_role = query.split("_")[1]
    query_color = role_colors[query_role]
    if "unseen_indistribution" in query:
        query_alpha = 0.8
    elif "unseen_middistribution" in query:
        query_alpha = 0.5
    elif "unseen_outofdistribution" in query:
        query_alpha = 0.2
    else:
        query_alpha = 1.0
    return query_color, query_alpha
def plot_split_data(data, epochs_dict, plot_index, experiment_title, experiment_title2, chance_rate, split_queries, save=False, save_dir=None):
    ...
    for i in range(len(split_queries)):
        ...
        query_color, query_alpha = get_color_alpha(query)
        bar_values = []
        bar_mins = []
        bar_maxs = []
        for architecture in architectures:
        ...
        xaxis_label = ["RNN\n%d epochs" % (epochs_dict["RNN-LN"]), "LSTM\n%d epochs" % (epochs_dict["LSTM-LN"]), "Fast Weights\n%d epochs" % (epochs_dict["RNN-LN-FW"]), "Reduced NTM\n%d epochs" % (epochs_dict["NTM2"])]
        ind = np.arange(len(xaxis_label))
        plt.bar(np.arange(xaxis_length) + split_width * i, bar_values, split_width, color=query_color, alpha=query_alpha,  yerr=(bar_mins, bar_maxs), label=query)
    plt.axhline(y=chance_rate, label="Chance Rate", color='k', linestyle='--')
    ...
    # plt.show()
    if save:
        plt.savefig(os.path.join(save_dir, 'multitrial_barchart_%s%s_%s_split' % (experiment_title.replace("_","").replace(" ","").replace(",",""), experiment_title2.replace("_","").replace(" ","").replace(",",""), info_type.replace(" ",""))), bbox_inches="tight", dpi=900, bbox_extra_artists=[legend])
    plt.close()
...
if __name__ == '__main__':
    ...
    print(trial_nums)
    SPLIT_TEMPLATE = "%s_results_%depochs_trial%d_tests.p"
